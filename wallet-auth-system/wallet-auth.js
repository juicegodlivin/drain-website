...
class WalletAuthController {
   ...
   */
  setupWalletButtons() {
    // Support both wallet-card (for auth system) and wallet-option (for custom popups)
    const walletButtons = document.querySelectorAll('.wallet-card, .wallet-option');
  ...
  async connectWallet(walletType) {
    ...
    const connectText = button.querySelector('.connect-text, .wallet-connect-btn');
    const originalName = walletName ? walletName.textContent : '';
    const originalConnect = connectText ? connectText.textContent : '';
    
    // Add loading state
    if (walletName) walletName.textContent = 'CONNECTING...';
    if (connectText) connectText.textContent = 'WAIT...';
    ...
    try {
      ...
      if (wallet) {
        ...
        // Show success state
        if (walletName) walletName.textContent = 'CONNECTED';
        if (connectText) connectText.textContent = 'SUCCESS';
      ...
      // Show error state  
      if (walletName) walletName.textContent = 'CONNECTION FAILED';
      if (connectText) connectText.textContent = 'ERROR';
      button.style.background = 'linear-gradient(135deg, rgba(255, 0, 64, 0.2), rgba(255, 0, 64, 0.1))';
      button.style.borderColor = '#ff0040';
      
      // Reset button after delay
      setTimeout(() => {
        if (walletName) walletName.textContent = originalName;
        if (connectText) connectText.textContent = originalConnect;
